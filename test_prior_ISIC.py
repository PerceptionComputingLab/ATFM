...
def test(
    ...
    for images, masks, _, _ in tqdm(testloader):
        ...
        for i in range(num_samples):
            ...
            #x0 = base_distribution.sample(sample_shape=(BS, 1, 256, 256)).to(device)
            x0 = torch.sigmoid(x0).to(device)
            result = euler_sampler(model=model, sampling_times=50, cond=images, x=x0, BS=BS)
        ...
        #IoU += compute_hm_iou(P2, Masks)
        #print(f'Current Mean HM-IoU = {IoU/tcnt}')
...
parser.add_argument("--output_path", type=str, default="results_ISIC")
...
if __name__ == "__main__":
    ...
    test_loader, _ = get_dataloader_2(
        task="isic3_style_concat", split="test", batch_size=1, shuffle=False, splitratio=[0.8, 0.0, 0.2], randomsplit=False
    )
    ...
    checkpoint = torch.load("/root/ATFM/saved_models/LIDC/120SFM-checkpoint.pth")
