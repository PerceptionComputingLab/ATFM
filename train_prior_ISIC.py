...
def train(
    ...
    for epoch in range(num_epochs - resume_epoch):
        ...
        for images, masks, _, _ in tqdm(dataloader):
            ...
            val = torch.tensor([0.299, 0.587, 0.114]).to(device)
            #print(images.shape)
            images = torch.sum(images * val.view(1, 3, 1, 1), dim=1, keepdim=True)
            x_1 = masks[0]
            ...
            #base_distribution = Normal(0, 1)
            #x_0_1 = base_distribution.sample(sample_shape=x_1.shape).to(device)
            ...
            loss3, dice3, iou3 = loss_fn(model=model, target_flow=target_flow, x_0=x_0_1, x_1=x_3, t=t, cond=images)
            ...
            dice = 0.33 * (dice1 + dice2 + dice3)
            iou = 0.33 * (iou1 + iou2 + iou3)
            Loss += loss
            Dice += dice
            IoU += iou
        ...
        torch.save(checkpoint, save_path / f"{epoch+resume_epoch}SFM-checkpoint.pth")
...
parser.add_argument("--output_path", type=str, default="/root/FlowMatchingLIDC/ISIC/0.0001/")
...
if __name__ == "__main__":
    ...
    train_loader, _ = get_dataloader_2(
        task="isic3_style_concat", split="train", batch_size=args.batch_size, shuffle=True, randomsplit=True
    )
    ...
    checkpoint1 = torch.load("/root/ATFM/saved_models/LIDC/GTR-ISIC.pt")
